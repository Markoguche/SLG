...
const C = { lime:'#B8FF00', gold:'#D4A843', dark:'#060F09', dark2:'#0c1a10', greenDeep:'#0A2E1A', greenMid:'#0F4A28', cream:'#F5F2EC', gray:'#8A9990', grayLight:'#b5c4bc' };

const partners = ['World Bank','UNDP','French Embassy','MacArthur Foundation','NITDA','IREX','AfriLabs','UNESCO','Luminate','JCI Abuja','FCT-DRTS','Convexity','Paxful','Neocloud','DIAL','World Bank','UNDP','French Embassy','MacArthur Foundation','NITDA','IREX','AfriLabs','UNESCO','Luminate','JCI Abuja','FCT-DRTS','Convexity','Paxful','Neocloud','DIAL'];
const values = [
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const pts = Array.from({ length: 60 }, () => ({ x: Math.random() * canvas.width, y: Math.random() * canvas.height, vx: (Math.random()-0.5)*0.25, vy: (Math.random()-0.5)*0.25, r: Math.random()*1.2+0.4, a: Math.random()*0.4+0.08 }));
    let raf;
    const draw = () => {
      ctx.clearRect(0,0,canvas.width,canvas.height);
      pts.forEach(p => { p.x+=p.vx; p.y+=p.vy; if(p.x<0)p.x=canvas.width; if(p.x>canvas.width)p.x=0; if(p.y<0)p.y=canvas.height; if(p.y>canvas.height)p.y=0; ctx.beginPath(); ctx.arc(p.x,p.y,p.r,0,Math.PI*2); ctx.fillStyle=`rgba(184,255,0,${p.a})`; ctx.fill(); });
      pts.forEach((a,i) => pts.slice(i+1).forEach(b => { const d=Math.hypot(a.x-b.x,a.y-b.y); if(d<110){ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.strokeStyle=`rgba(184,255,0,${0.07*(1-d/110)})`;ctx.stroke();} }));
  ...
  return (
    <>
      ...
      <section ref={heroRef} style={{position:'relative',minHeight:'100vh',display:'flex',alignItems:'center',overflow:'hidden',background:C.dark}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:0,alignItems:'center',minHeight:'70vh'}} className="hero-cols">
            <div style={{paddingRight:'clamp(0px,3vw,40px)'}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'1px',marginTop:'48px',background:'rgba(255,255,255,0.04)',maxWidth:'420px'}}>
                {[{num:103500,suffix:'+',label:'People Impacted'},{num:6200,suffix:'+',label:'Tech Professionals'},{num:350,suffix:'K+',label:'USD Managed'},{num:6,suffix:'+',label:'Businesses Built'}].map(({num,suffix,label})=>(
                  <div key={label} className="hero-stat" style={{background:'rgba(6,15,9,0.9)',padding:'clamp(14px,2vw,22px)',backdropFilter:'blur(12px)',borderTop:'2px solid transparent',transition:'border-color 0.3s'}}
                    onMouseEnter={e=>e.currentTarget.style.borderTopColor=C.lime} onMouseLeave={e=>e.currentTarget.style.borderTopColor='transparent'}>
        ...
        <style>{`@media(max-width:900px){.hero-cols{grid-template-columns:1fr!important;}.hero-robot{height:clamp(240px,55vw,380px)!important;}}@media(max-width:640px){.hero-robot{height:clamp(200px,70vw,300px)!important;opacity:0.85;}}`}</style>
      </section>

      {/* MARQUEE */}
      <div className="marquee-wrap"><div className="marquee-track">{partners.map((p,i)=><div key={i} className="marquee-item"><span style={{color:C.lime,fontWeight:700,fontSize:'0.6rem'}}>✦</span>{p}</div>)}</div></div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'clamp(36px,6vw,96px)',alignItems:'center'}} className="about-grid">
            ...
            <div className="abt-visual" style={{position:'relative'}}>
              <div style={{background:C.greenDeep,padding:'clamp(28px,4vw,48px)',border:'1px solid rgba(184,255,0,0.1)',position:'relative',overflow:'hidden'}}>
                {[{label:'Vision',color:C.lime,text:'To be a leading catalyst for global business collaboration, recognized for transforming opportunities into impactful partnerships that contribute to economic development worldwide.'},{label:'Mission',color:C.gold,text:'To empower business by providing development solutions and strategic partnerships that drive growth, enhance market presence and foster sustainable success for local and international stakeholders.'}].map(({label,color,text},i)=>(
      ...
      <section className="svc-sec section-pad" style={{background:C.dark2}}>
        <div className="container">
          <div className="svc-hdr" style={{display:'flex',justifyContent:'space-between',alignItems:'flex-end',flexWrap:'wrap',gap:'20px',marginBottom:'clamp(36px,5vw,64px)'}}>
            <div><div className="tag" style={{marginBottom:'18px'}}>What We Do</div><h2>Our Core <em style={{color:C.lime,fontStyle:'italic'}}>Services</em></h2></div>
          ...
          <div className="svc-grid" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'1px',background:'rgba(255,255,255,0.04)'}}>
            {[{num:'01',title:'AI-Driven Business Development',sub:'AI-BaaS',desc:'Integrating AI into operations, marketing, finance and decision-making to drive revenue and efficiency.',color:C.lime},{num:'02',title:'Venture Studio Partnership',sub:'Venture Studio',desc:'Helping startups build, go to market, and scale — connecting them to the funding and support they need.',color:C.gold},{num:'03',title:'Event & Program Management',sub:'Events',desc:'World-class event execution from intimate workshops to 2,000+ person conferences with measurable impact.',color:'#7EB8FF'},{num:'04',title:'Training & Capacity Building',sub:'Training',desc:'Empowering talent through 75+ sessions delivered, 500+ trained tech professionals, and career placement.',color:'#FF9B7A'}].map(({num,title,sub,desc,color})=>(
              <div key={num} className="svc-card" style={{background:C.dark,padding:'clamp(28px,4vw,52px)',position:'relative',transition:'background 0.35s',overflow:'hidden',cursor:'default'}}
                ...
                onMouseLeave={e=>{e.currentTarget.style.background=C.dark;e.currentTarget.querySelector('.svc-line').style.width='0%';}}>
      ...
      <section className="stats-sec section-pad" style={{background:C.greenDeep,position:'relative',overflow:'hidden'}}>
        ...
        <div className="container" style={{position:'relative',zIndex:1}}>
          ...
          <div className="stats-grid" style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'clamp(16px,2.5vw,36px)'}}>
            {[{num:103500,suffix:'+',label:'People Impacted',detail:'Trainings, projects & programs'},{num:6200,suffix:'+',label:'Tech Professionals',detail:'In our talent pool'},{num:75,suffix:'+',label:'Training Sessions',detail:'Delivered across Africa'},{num:350000,suffix:'+',label:'USD Managed',detail:'In development funds'},{num:500,suffix:'+',label:'Trained Talents',detail:'Graduated and placed'},{num:350,suffix:'+',label:'Talents Hired',detail:'By partner organizations'},{num:30,suffix:'+',label:'Conferences',detail:'And corporate events'},{num:6000,suffix:'',label:'Beneficiaries',detail:'Direct program graduates'}].map(({num,suffix,label,detail})=>(
        ...
        <div className="container">
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'clamp(32px,5vw,80px)',alignItems:'center'}} className="amb-grid">
            <div className="amb-text">
              <div className="tag" style={{marginBottom:'20px',borderColor:'rgba(212,168,67,0.4)',color:C.gold}}>Recognition</div>
              <h2 style={{marginBottom:'20px'}}>US Ambassador<br/><em style={{color:C.gold,fontStyle:'italic'}}>Visits SLG HQ</em></h2>
              ...
              <Link href="/about" className="btn btn-outline" style={{borderColor:C.gold,color:C.gold}}>Our Story <IconArrowRight size={16} color={C.gold}/></Link>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
              {['amb1.jpg','amb2.jpg','amb3.jpg','amb4.jpg','amb5.jpg','amb6.jpg','amb7.jpg','amb8.jpg'].map((img,i)=>(
                <div key={img} className="amb-img img-zoom" style={{aspectRatio:i===0?'16/10':'1/1',gridColumn:i===0?'1/-1':'auto',background:C.greenMid,overflow:'hidden',border:'1px solid rgba(212,168,67,0.15)',animation:`${i%2===0?'floatY':'floatY2'} ${5+i}s ease-in-out infinite`}}>
                  <img src={`/${img}`} alt={`US Ambassador visit ${i+1}`} style={{width:'100%',height:'100%',objectFit:'cover'}} onError={e=>{e.target.style.display='none';e.target.parentNode.style.background='rgba(184,255,0,0.06)';}}/>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'clamp(14px,2vw,22px)'}} className="vals-grid">
            {values.map(({Icon,name,desc})=>(
              <div key={name} className="card val-card" style={{textAlign:'center',padding:'clamp(24px,3vw,40px)'}}>
                <div style={{width:'52px',height:'52px',background:'rgba(184,255,0,0.07)',border:'1px solid rgba(184,255,0,0.2)',display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 16px'}}><Icon size={24} color={C.lime}/></div>
                <h4 style={{fontFamily:'var(--font-body)',fontWeight:700,fontSize:'1rem',color:C.lime,marginBottom:'10px',letterSpacing:'0.02em'}}>{name}</h4>
      ...
      <section className="cta-sec section-pad" style={{background:C.dark,position:'relative',overflow:'hidden'}}>
        ...
        <div className="container cta-inner" style={{position:'relative',zIndex:1,textAlign:'center'}}>
          ...
          <p style={{maxWidth:'480px',margin:'0 auto 44px',fontSize:'clamp(0.98rem,1.25vw,1.15rem)',color:C.grayLight}}>Whether you're a startup seeking funding, a business looking to integrate AI, or an organization ready to scale — we're your partner for sustainable growth.</p>
          ...
          <div style={{display:'flex',justifyContent:'center',gap:'clamp(28px,5vw,56px)',marginTop:'56px',paddingTop:'40px',borderTop:'1px solid rgba(255,255,255,0.05)',flexWrap:'wrap'}}>
            {[{label:'40+',sub:'Global Partners'},{label:'10+',sub:'Businesses Set Up'},{label:'3K+',sub:'RescueTap Users'}].map(({label,sub})=>(
              <div key={label} style={{textAlign:'center'}}>
                <div style={{fontFamily:'var(--font-display)',fontSize:'clamp(1.5rem,3vw,2rem)',fontWeight:700,color:C.gold}}>{label}</div>
